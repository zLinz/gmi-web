...
RunServer.startServe = function () {
    ...
    app.listen (9090)
    var browserObject = opener ("http://localhost:9090")
...
RunServer.renderSass = function () {
    setTimeout ( () => {
        sass.render ({
            ...
            console.log ("Render sass: " + result.stats.duration)
...
RunServer.watch = function () {
    chokidar.watch ('src', {
        ...
        } else if (/^src(\\|\/)js(\\|\/)lib.*\.js$/.test (path)) {
            fsExtra.copy (path, path.replace ('src', 'dist'), (err) => {
                if (err) console.log (err)
                console.log (path + " copied!") 
            })
        } else if (/^src(\\|\/)scss(\\|\/)lib.*/.test (path)) {
            fsExtra.copy (path, path.replace ('src', 'dist').replace ('scss', 'css'), (err) => {
                if (err) console.log (err)
                console.log (path + " copied!") 
            })
        }
...
function renderRecursive (path, content) {
    ...
    if (/<\%\ \@\@include="(.*?)"\ \%>/.test (content)) {
        ...
        let filename = path.replace(/^src/, 'dist')
